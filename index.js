import{a as x,S as R,N as U,P as _,A as Y}from"./assets/vendor-SkZP4jF-.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))d(i);new MutationObserver(i=>{for(const l of i)if(l.type==="childList")for(const s of l.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&d(s)}).observe(document,{childList:!0,subtree:!0});function a(i){const l={};return i.integrity&&(l.integrity=i.integrity),i.referrerPolicy&&(l.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?l.credentials="include":i.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function d(i){if(i.ep)return;i.ep=!0;const l=a(i);fetch(i.href,l)}})();const E=document.querySelector(".mobile-menu"),G=document.querySelector(".js-mobile-menu"),K=document.querySelector(".js-mobile-close-menu"),A=document.querySelector(".js-mobile-backdrop");function z(){E.classList.add("is-open"),A.classList.add("is-active"),document.body.classList.add("no-scroll")}function b(){E.classList.remove("is-open"),A.classList.remove("is-active"),document.body.classList.remove("no-scroll")}G.onclick=z;K.onclick=b;A.onclick=b;document.addEventListener("keydown",e=>{e.key==="Escape"&&b()});E.querySelectorAll('a[href^="#"]').forEach(e=>{e.onclick=b});const w=x.create({baseURL:"https://sound-wave.b.goit.study/api",timeout:5e3,headers:{"Content-Type":"application/json"}});w.interceptors.response.use(e=>e,e=>(alert("Сталася помилка при запиті до сервера. Спробуйте ще раз."),Promise.reject(e)));async function J(){try{return(await w.get("/artists")).data}catch(e){throw e}}async function Q(e){try{return(await w.get(`/artists/${e}/albums`)).data}catch(t){throw t}}const $=document.querySelector("[data-modal]"),I=document.getElementById("modal-inner-content"),P=document.querySelector("[data-modal-backdrop]"),q=document.getElementById("loader");function V(){q.classList.remove("is-hidden")}function C(){q.classList.add("is-hidden")}function W(){$.classList.remove("is-hidden"),document.body.style.overflow="hidden",document.addEventListener("keydown",N)}function k(){$.classList.add("is-hidden"),document.body.style.overflow="",document.removeEventListener("keydown",N),I.innerHTML=""}function N(e){e.key==="Escape"&&k()}function X(e){e.target===P&&k()}$.addEventListener("click",e=>{e.target.closest("[data-modal-close]")&&k()});P.addEventListener("click",X);function Z(e){const t=Math.floor(e/1e3),a=Math.floor(t/60),d=t%60;return`${a}:${d.toString().padStart(2,"0")}`}async function ee(e){try{V();const t=await Q(e);C(),te(t)}catch(t){C(),console.error("Error fetching artist:",t),alert("Не вдалося завантажити інформацію про виконавця.")}}function te(e){const{strArtist:t,strArtistThumb:a,intFormedYear:d,intDiedYear:i,strGender:l,intMembers:s,strCountry:c,strBiographyEN:o,genres:r,albumsList:u}=e,p=s===1?`<li><strong>Sex:</strong> ${l||"—"}</li>`:`<li><strong>Members:</strong> ${s||"—"}</li>`,j=`
    ...
    </ul>`,M=(r==null?void 0:r.map(v=>`<li>${v}</li>`).join(""))||"",D=(u==null?void 0:u.map(v=>{const{strAlbum:H,tracks:O}=v,B=O.map(f=>{const F=f.movie&&f.movie!=="null"?`<a href="${f.movie}" target="_blank" rel="noopener">
